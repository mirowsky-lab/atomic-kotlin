package dev.mirowsky.sectionTwo.atomicTest
...
private fun <L, R> test(actual: L, expected: R, checkEquals: Boolean = true, predicate: () -> Boolean) {
    println(actual)
    if (predicate()) {
        print(ERROR_TAG)
        println("$actual " +
...
infix fun <TType> TType.eq(rval: TType) {
    test(this, rval) { this == rval}
}

infix fun <TType> TType.neq(rval: TType) {
    test(this, rval, checkEquals = false) { this != rval}
}

infix fun Double.eq(rval: Double) {
    test(this, rval) { abs(this - rval) < 0.0000001 }
}



class CapturedException(private val exceptionClass: KClass<*>?, private val actualMessage: String) {

    private val fullMessage: String get() {
